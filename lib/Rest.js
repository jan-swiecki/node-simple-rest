...
var handlerFactory = function(method) {
  return function(path, callback){
    ...
    callback = this.injector.attach(callback).autoWireModules();
  ...
  return function(req, res) {
    self.requestRouter.findHandlerForRequest(req, res)
      ...
      .spread(function(handler, pathVarsMap) {
          var result = handler.callback.executeInject(pathVarsMap);

          return self.promiseResponse(result, handler.callback.originalName).then(function(response){
            response.write(res);
          });
      })
